...
1. 事件定位：每个事件是玩家这一天中「印象最深的一件事」——不必是大事，但必须有强烈的情感张力，情感张力可以来自自我挣扎、时间取舍、意外状况等多种来源，不必依赖同一段人物关系反复发酵。现实本就足够多彩。不迎合大众刻板印象，注重非常规但又符合现实的事件，那些新生预测大学生活时常被忽略的事件。参考过往经历，事件要体现出起伏，不能都是好事，也不能都是坏事。
2. 时间跨度：事件发生在这一天中一个具体时段的具体场景（如午休时的宿舍、晚自习后的操场、清晨的食堂），description 中要给出具体时间与场景；不要两天都讲同一天内的事；不要写成跨越多天的总结。
