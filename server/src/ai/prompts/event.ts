...
// 提示词草案 v0.3.4：v0.3 事件定位改为「一天中印象最深的事」+强情感张力、游戏更名《新生每日印象》（迭代 6）；v0.3.1 增补背景设定（军训在两周之后、学期开端无期末）与金钱净额一致性；v0.3.2 增补玩家自主权（不预设爱好偏好、未选的决定不得成为既成事实）；v0.3.3 增补生活广度（张力来源多样化、整局侧面覆盖、NPC 出场轮换、社团成员身份边界、用户消息动态广度提示）；v0.3.4 修正过矫正与时间错乱（盲区提示降载并限前半局、词表去交叠、侧面级上限、历史条目标注距今天数、引用过往须核对间隔）；v0.3.5 玩家档案改为可选，未收集时用中性表述（随实测迭代）；游戏名后随项目统一为《我的大学日记》
