...
- 9.13：负责人在 main 直接改了本模块 4 个文件，已合入 feat/AI（合并提交 `bcb0a9f`；唯一冲突在 ai/README.md 错误处理段，按新规则手解并保留 `[ai]` 日志说明）。四项变化：①**事件生成改单次调用**——`generateEvent` 不再走 `chatJSONWithRetry`，校验失败立即抛错、由玩家手动重试；`generateEventStream` 的重试循环与 `onRetry` 回调一并移除；`AI_MAX_ATTEMPTS` 自动重试仅保留给结局；②`validate.ts` effects 校验放宽输入：纯数字字符串与范围内小数按绝对值四舍五入规范化为整数，缺失、非数值、越界仍然拒绝（取整不得掩盖越界）；③事件提示词硬性约束同步收紧：effects 必须完整含四字段、JSON 整数、禁止字符串/小数/null；④`server/src/ai/README.md` 错误处理与流式两节按新规则重写。配套：controller 删 `onRetry`→`reset` 帧，集成测试断言「校验失败 / 上游故障只调用一次模型」。本地同批：事件提示词第 5 条就地重申取值范围（引 `EFFECT_RANGES` 不写死数字）。
...
`generateEventStream(input, cfg, handlers)`（已从 ai 出口导出）：`generateEvent` 的流式版，对模型发 `stream: true`。**单次调用，无重试**：校验失败或临时错误直接抛出，已流出的字不重写，由玩家手动重试。流式面：描述逐字（`onDescriptionDelta`）、结果叙述逐字（`onResultTextDelta` 增量 + `onResultText` 闭合）；标题与选项文本整体回调（`onTitle` 一次给出，`onOptionText` 每选项整条弹出，A→B→C 依次）；effects 是结算数值，不流式。id 由服务端分配。最终返回值与 `generateEvent` 完全一致（同一套整体校验与选项字母 id），回调推送的内容尚未通过最终校验，一切以最终返回的 `GameEvent` 为准。流式过程中即时拦截提示词硬禁词（军训 / 期末），命中立即中止本次生成并抛错；结构偏离预期时增量解析自动休眠，正确性由整体校验兜底。

转发协议（9.13 起**已实现**，此前草案作废）：`POST /api/events/generate` 返回 NDJSON（`application/x-ndjson`），每行一个 JSON 帧：

| 帧 | 语义 |
| --- | --- |
| `{"k":"title","v":"…"}` | 标题整体（定稿信号） |
| `{"k":"desc","v":"…"}` | 描述增量（已反转义） |
| `{"k":"end",…}` | 终帧：成功 = 完整 `SuccessResponse` 信封（含新快照，以此为准替换）；失败 = 附 `error` 字段（错误码映射不变）。流开始后所有错误走终帧、HTTP 200 |

controller 目前只转发 `onTitle` / `onDescriptionDelta` 两个回调；`onOptionText` / `onResultTextDelta` / `onResultText` 在 handlers 接口里但暂不写帧（前端暂不画），选项文本与结果叙述随 `end` 终帧的完整快照到达。前端解析在 `client/src/api/frames.ts`（`createFrameParser`，兼容分片与末尾无换行，集成测试覆盖）。

流式版超时按「静默时长」计（沿用 `AI_TIMEOUT_MS`），模型持续出字不算超时。离线回归解析器（不调 API）：`npx tsx src/ai/scripts/stream-parse-test.ts`。演示脚本（真实调用）：`npx tsx src/ai/scripts/stream-demo.ts`。
