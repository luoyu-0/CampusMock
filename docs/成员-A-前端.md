...
25. **第 14 天选完选项会永久卡在「正在为你写结尾」，两种模式都卡**（给成员 B，也报负责人）。证据链：`server/src/game/controller.ts:199-200` 让第 14 次 `choose` 的返回 phase **直接是 `pendingEnding`**（不是 `showResult`），mock 侧同构（`client/src/api/script.ts:302`）；而客户端 `dispatch` 的成功路径只做 `persistAndApply(result.snapshot)`（`useGame.ts:179`），`resume` 默认 `'none'`，**没有任何地方为这个响应发起 `generateEnding`**；`GamePage.tsx:56` 在 `pendingEnding` 只渲染 `EndingWaitingCard`，那张卡上唯一那颗按钮是 `disabled` 的（`Cards.tsx:47`）。于是玩家点完第 14 天的选项就停在一个**没有请求在飞、也没有按钮可点**的骨架屏上，刷新也救不回来（挂载恢复走 `applySnapshot`，同样不 `stepFrom`，`useGame.ts:196-198`），只有退回开始页点「继续上次的日记」才会通过 `resume()` → `stepFrom()` 踢出去。之前一直没暴露是因为走查面板的「跑完整 14 天」是 `runFullWalk` 直接 `await dispatch(..., generateEnding)`（`useGame.ts:303`），绕过了 UI。**两种修法，归属不同**：① 我在客户端补一次 `stepFrom`（一行，我这侧，代价是玩家看不到第 14 篇的结算文案——`ResultCard` 只在 `showResult` 渲染）；② B 把第 14 天的 `choose` 改回返回 `showResult`，由我那句「去写结尾」按钮触发结局（`GamePage.tsx:48`，符合我原来的设计，但那是他的文件）。**我和 B 对"谁来触发结局"的理解本来就不一致，这条不是单方面写错**。我先不动，等一句定夺。
26. **流式渲染的帧约定已经设计好，按用户决定未开工，等 B / C 认领**。目标是把"模型逐字生成十几秒"变成"字在动"，前端这侧随时可以做，但**只改前端做不出流式**：`deepseek.ts:109` 是 `await res.json()` 整包读完，`controller.ts` 用 `sendSuccess` 一次写完，上游也没开 `stream`。约定我定成这样，谁接手照抄即可：传输用 NDJSON（`POST` 响应一行一个 JSON，`Content-Type: application/x-ndjson`，不用 `EventSource` 因为它带不了 POST body）；**终帧就是今天那个完整的成功响应，一个字段都不改，只多一个 `"k":"done"`**，所以五要素比对、只在完整响应后写档、重试幂等这些语义全部原样保留；中间帧只有 `{"k":"delta","t":"上游片段"}` 与 `{"k":"reset"}`（服务端要重试了，前端抹掉已显示的字）；**C 只管把上游 `choices[0].delta.content` 原样转发、不解析 JSON**，从残缺 JSON 里提取 `title` / `description` 的正则归我写；前端两种响应都吃（`content-type` 是 JSON 就按今天的路子走），所以你们来不及我这边也不会退不回去。附带一条：`chatJSONWithRetry`（`deepseek.ts:134-153`）整个重试循环一行日志都没有，所以现在无法从后端控制台判断"慢"是不是重试在跑——加一行 `console.warn` 只要 30 秒，这条比流式更该先做。
